...
func TestJsonDataInterface(t *testing.T) {
	...
	*/
	...
	if result.ResultType == message.ResultTypeSuccess {
		...
		// 注意 result.Date 是接口类型，所以必须用指针类型进行赋值才可以断言类型得到正确结果即：&SuccessData{},且 &message.CallbackOrderTaskResponse
		// 正确
		...
		/*// 错误：原因 result.Date 是接口类型，所以必须用指针类型进行赋值才可以断言类型得到正确结果即：&SuccessData{},且 &message.CallbackOrderTaskResponse
		result.Date = &message.SuccessData{}
		err = json.Unmarshal([]byte(data), result)
		if err != nil {
			log.Fatal("解析 data CallbackOrderTaskResponse.SuccessData 1 错误:", err)
		}
		resultTypeSuccessData1, resultTypeSuccessDataOk := result.Date.(message.SuccessData)
		if !resultTypeSuccessDataOk {
