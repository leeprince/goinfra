...
type RedisClient interface {
    ...
    // 字符串：将值 value 关联到 key ，并将 key 的过期时间设为 seconds (以秒/毫秒为单位)。
    SetNx(key string, value interface{}, expiration time.Duration) (bool, error)
    // 字符串：获取指定键名，并将键值与 value 对比，一致则删除，是通过 lua 脚本执行，做到原子性
    GetAndDel(key string, value interface{}) error
    // 字符串：获取指定键名的字符串
    GetString(key string) string
    // 字符串：获取指定键名的字节切片
    GetBytes(key string) ([]byte, error)
    // 字符串：获取指定键名的键值，并转化为指定数据结构
    GetScanStruct(key string, value interface{}) error
    // 列表：将一个或多个值插入到列表头部（最左，默认）或者尾部（最右）
    Push(key string, value interface{}, isRight ...bool) error
    // 列表：移除列表的最左或者最右（默认）一个元素，返回值为移除的元素。
    Pop(key string, isLeft ...bool) (data []byte, err error)
    // 列表：移出并获取列表的第一个元素， 如果列表没有元素会阻塞（设置堵塞时间，不建议设置为0，防止堵塞时间过长，redis 服务端主动断开连接，导致数据丢失）列表直到等待超时或发现可弹出元素为止。
    BPop(key string, timeout time.Duration, isLeft ...bool) (data interface{}, err error)
    // 有序集合：向有序集合添加一个或多个成员，或者更新已存在成员的分数。完整的命令：ZADD key [NX|XX] [CH] [INCR] score member [score member ...] 可选项都是默认的
    ZAdd(key string, members ...*Z) error
    // 有序集合：通过分数返回有序集合指定区间内的成员(默认从低到高排序，从高到低排序需使用：ZREVRANGEBYSCORE)
