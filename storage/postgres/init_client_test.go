...
func TestInitMysqlClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := InitPostgresClientMap(tt.args.confs); (err != nil) != tt.wantErr {
				t.Errorf("InitPostgresClientMap() error = %v, wantErr %v", err, tt.wantErr)
			}
