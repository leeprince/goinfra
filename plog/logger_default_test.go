...
func TestDefaultLoggerStruct(t *testing.T) {
	// 测试方法在当前包中，不会执行init() 方法，手动设置默认的logger==init()
	NewDefaultLogger()
	...
	WithField("person", person).Info("TestDefaultLoggerStuct")
	WithField("person", person).Infof("TestDefaultLoggerStuct: %v", person)
}
