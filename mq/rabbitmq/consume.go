		...
		for {
			...
			case data := <-delivery:
				...
				if data.Acknowledger == nil && string(data.Body) == "" {
					plog.Info("<-delivery: data.Acknowledger == nil && string(data.requestBody) == ''")
					err = cli.Consume(handle, opts...)
					failOnError(err, "<-delivery: data.Acknowledger == nil && string(data.requestBody) == '' > cli.Consume(handle, opts...) err")
					return
				}
				go handle(data)
			}
